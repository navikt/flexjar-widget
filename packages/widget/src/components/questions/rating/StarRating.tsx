...
export function StarRating({
    ...
    const labels = question.labels
        ? question.labels.reduce<Record<number, string>>(
            (acc, label) => ({ ...acc, [label.value]: label.label }),
            {},
        )
        : STAR_LABELS.reduce<Record<number, string>>(
            (acc, label, index) => ({ ...acc, [index + 1]: label }),
            {},
        );
    ...
    const handleKeyDown = useCallback(
        ...
        [activeState, scale, handleSelect],
    );
            ...
            >
                ...
                <VStack gap="space-4" align="center">
                        ...
                        {Array.from({ length: scale }, (_, index) => {
                            ...
                            return (
                                <button
                                    ...
                                    disabled={disabled}
                                    tabIndex={isActive || (!activeState && starValue === 1) ? 0 : -1}
                                    style={{
                                        display: "flex",
                                        alignItems: "center",
                    ...
                    {showLabels === 'always' && (
                        ...
                        >
                            {activeState ? (labels[activeState] ?? `${activeState} av ${scale}`) : `Velg 1-${scale}`}
                        </BodyShort>
                    )}
