...
function compareValues(
    ...
    switch (operator) {
        case "EQ":
            ...
            return actual == expected; // Loose equality for string/number comparison
 ...
 */
function findQuestionIndex(questions: FlexJarQuestion[], targetId: string): number {
        ...
        if (matches) {
            switch (rule.action.type) {
                case "JUMP_TO": {
                    ...
                    const targetIndex = findQuestionIndex(questions, rule.action.targetId);
                    if (targetIndex === -1) {
 ...
 */
export function validateBranchingTargets(questions: FlexJarQuestion[]): string[] {
