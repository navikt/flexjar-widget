...
describe("evaluateBranching", () => {
    describe("default behavior (no logic)", () => {
        it("returns next index when no logic is defined", () => {
            const questions = [createQuestion("q1"), createQuestion("q2"), createQuestion("q3")];
            const result = evaluateBranching(questions[0], "some answer", undefined, questions, 0);
        ...
        it("returns next index when logic array is empty", () => {
            ...
            const result = evaluateBranching(questions[0], "answer", undefined, questions, 0);
    ...
    describe("ANSWER-based conditions", () => {
        it("EQ: jumps to target when answer matches", () => {
            ...
            const result = evaluateBranching(questions[0], "YES", undefined, questions, 0);
        ...
        it("EQ: goes to next when answer does not match", () => {
            ...
            const result = evaluateBranching(questions[0], "NO", undefined, questions, 0);
        ...
        it("NEQ: jumps when answer is different", () => {
            ...
            const result = evaluateBranching(questions[0], "SAD", undefined, questions, 0);
        ...
        it("GT: works with numeric rating", () => {
            ...
            expect(evaluateBranching(questions[0], 5, undefined, questions, 0).nextIndex).toBe(2);
            expect(evaluateBranching(questions[0], 4, undefined, questions, 0).nextIndex).toBe(2);
            expect(evaluateBranching(questions[0], 3, undefined, questions, 0).nextIndex).toBe(1);
            expect(evaluateBranching(questions[0], 2, undefined, questions, 0).nextIndex).toBe(1);
        });
        ...
        it("LT: works with numeric rating", () => {
            ...
            expect(evaluateBranching(questions[0], 1, undefined, questions, 0).nextIndex).toBe(2);
            expect(evaluateBranching(questions[0], 2, undefined, questions, 0).nextIndex).toBe(2);
            expect(evaluateBranching(questions[0], 3, undefined, questions, 0).nextIndex).toBe(1);
        });
        ...
        it("CONTAINS: works with text answers", () => {
            const logic: LogicRule[] = [
                {
                    condition: { field: "ANSWER", operator: "CONTAINS", value: "problem" },
            ...
            expect(
                evaluateBranching(questions[0], "I have a problem with login", undefined, questions, 0)
                    .nextIndex
            ).toBe(2);
            expect(
                evaluateBranching(questions[0], "Everything is fine", undefined, questions, 0).nextIndex
            ).toBe(1);
        });
        ...
        it("EQ: works with multiChoice arrays", () => {
            ...
            expect(
                evaluateBranching(questions[0], ["urgent", "bug"], undefined, questions, 0).nextIndex
            ).toBe(2);
            // Array does not include "urgent"
            expect(
                evaluateBranching(questions[0], ["bug"], undefined, questions, 0).nextIndex
            ).toBe(1);
        });
    });
    ...
    describe("METADATA-based conditions", () => {
        it("EQ: jumps based on metadata value", () => {
            const logic: LogicRule[] = [
                {
                    condition: { field: "METADATA", key: "userRole", operator: "EQ", value: "admin" },
            ...
            expect(
                evaluateBranching(questions[0], "any", adminMetadata, questions, 0).nextIndex
            ).toBe(2);
            expect(
                evaluateBranching(questions[0], "any", userMetadata, questions, 0).nextIndex
            ).toBe(1);
        });
        ...
        it("EQ: handles boolean metadata", () => {
            const logic: LogicRule[] = [
                {
                    condition: { field: "METADATA", key: "harDialogmote", operator: "EQ", value: true },
                    action: { type: "SKIP" },
                },
            ];
            const questions = [createQuestion("q1", logic), createQuestion("q2"), createQuestion("q3")];

            expect(
                evaluateBranching(questions[0], "any", { harDialogmote: true }, questions, 0).nextIndex
            ).toBe(2); // SKIP goes to +2
            expect(
                evaluateBranching(questions[0], "any", { harDialogmote: false }, questions, 0).nextIndex
            ).toBe(1);
        });
        ...
        it("handles missing metadata key gracefully", () => {
            const logic: LogicRule[] = [
                {
                    condition: { field: "METADATA", key: "nonexistent", operator: "EQ", value: "test" },
                    action: { type: "JUMP_TO", targetId: "q3" },
                },
            ];
            const questions = [createQuestion("q1", logic), createQuestion("q2"), createQuestion("q3")];

            expect(evaluateBranching(questions[0], "any", {}, questions, 0).nextIndex).toBe(1);
            expect(evaluateBranching(questions[0], "any", undefined, questions, 0).nextIndex).toBe(1);
        });
    });
    ...
    describe("action types", () => {
        it("SKIP: skips next question", () => {
            ...
            const result = evaluateBranching(questions[0], "SKIP", undefined, questions, 0);
        ...
        it("SUBMIT: returns -1 to signal submission", () => {
            ...
            const result = evaluateBranching(questions[0], 5, undefined, questions, 0);
    ...
    describe("rule priority", () => {
        it("first matching rule wins", () => {
            ...
            const result = evaluateBranching(questions[0], "A", undefined, questions, 0);
    ...
    describe("error handling", () => {
        it("ignores JUMP_TO with invalid targetId (target doesn't exist)", () => {
            ...
            const questions = [createChoiceQuestion("q1", ["TEST"], logic), createQuestion("q2")];

            const result = evaluateBranching(questions[0], "TEST", undefined, questions, 0);
